...
def crawl(url, q):
    ...
    headers = ua()
    while q.empty():
        print('ip proxy is empty')
        time.sleep(20)
    proxies = q.get()
    try:
        r = requests.get(url, headers=headers, proxies=proxies, timeout=5).content
        sel = etree.HTML(r)
        q.put(proxies)
        pngs = sel.xpath('//img/@src')
        for png in pngs:
            if 'http:' not in png:
                png = 'http:' + png
            r = requests.get(png, headers=headers).content
...
def crawl_run(q):
    spawns = []
    for ur in ('http://jandan.net/ooxx/page-%s#comments' % n for n in range(2299)):
        spawns.append(gevent.spawn(crawl, ur, q))
    gevent.joinall(spawns)
